...
public class IntentReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		//Log.d(LEDIActivity.TAG, action);
		
		/*
		Bundle b = intent.getExtras();
		for (String key : b.keySet()) {
			Log.d(LEDIActivity.TAG, "extra: " + key);
        }		
		Log.d(LEDIActivity.TAG, intent.getDataString());
		*/
		
		if (action.equals("android.intent.action.PROVIDER_CHANGED")) {
			
			if (!LEDIService.Preferences.notifyGmail)
				return;
			...
			if (!Utils.isGmailAccessSupported(context)) {
				...
				Log.d(LEDIActivity.TAG, "count for " + recipient + ": " + bundle.getInt("count"));
		...
		if (action.equals("android.provider.Telephony.SMS_RECEIVED")) {		
			...
			if (!LEDIService.Preferences.notifySMS)
				return;
		...
		if (action.equals("com.fsck.k9.intent.action.EMAIL_RECEIVED")) {
			...
			if (!LEDIService.Preferences.notifyK9)
				return;
		...
		if (action.equals("com.android.alarmclock.ALARM_ALERT") || action.equals("com.htc.android.worldclock.ALARM_ALERT") || action.equals("com.android.deskclock.ALARM_ALERT") || action.equals("com.sonyericsson.alarm.ALARM_ALERT") ) {
			...
			if (!LEDIService.Preferences.notifyAlarm)
				return;
		...
		{	
			if (!LEDIService.Preferences.notifyMusic)
				return;
